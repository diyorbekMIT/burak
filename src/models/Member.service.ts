...
class MemberService {
    ...
    public async login(input: LoginInput): Promise<Member> {
        const member = await this.memberModel
            .findOne({ memberNick: input.memberNick }, { memberNick: 1, memberPassword: 1, MemberStatus: 1 })
            .exec();

        if (!member) {
            throw new Errors(HttpCode.NOT_FOUND, Message.NO_ERROR_NICK);
        }
    ...
    public async processLogin(input: LoginInput): Promise<Member> {
        ...
        if (!member) {
            throw new Errors(HttpCode.NOT_FOUND, Message.USED_NICK_PHONE);
        }
